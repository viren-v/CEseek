CEseek_scanner_anchored <- function(input.seqs, tf.pwms, length.gap, half.gap, match.thres, anchor.tf.string) {
le.gap <- length.gap
#########################################################################################
...
makeCEframe <- function(scaned_out, halfgap, pwms, scanned.seqs, le.gap, anchor.tf.string){
	...
	tfs <- names(pwms)
	c2.a <- combn(tfs, 2)
	c2 <- c2.a[,union(grep(anchor.tf.string, c2.a[1,]), grep(anchor.tf.string, c2.a[2,]))]
	 c2.list <- lapply(apply(c2,2, as.list), unlist)
	 #c2.list <- c2.list[sapply(c2.listed, function(x) length(grep("anchor.tf.string", x)))>0]
	...
	#c2.2.frame <- c2.2.frame[grep(anchor.tf.string, names(c2.2.frame))]
	#c2.2.frame.seqs <- c2.2.frame.seqs[grep(anchor.tf.string, names(c2.2.frame.seqs))]
...
#saveRDS(out.1, "out.1.RDS")
out.2 <- makeCEframe(out.1, half.gap, tf.pwms, input.seqs, le.gap, anchor.tf.string)
